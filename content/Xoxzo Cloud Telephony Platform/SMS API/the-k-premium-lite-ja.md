...
Kプレミアム Lite とは、日本の国内キャリア au (KDDI) および NTTドコモ のエンドユーザ様にショートメッセージを配信するために特化した、オプションのサービスです。キャリア側の制限により配信したSMSが届きにくかったり、受信したメッセージに設定通りのSender IDが表示されなかったりすることがありますが、Kプレミアム Lite は キャリア直収のため、配信の確実性が上がるのに加え、事前登録いただいたご指定の送信元IDを使った送信が可能となります。
