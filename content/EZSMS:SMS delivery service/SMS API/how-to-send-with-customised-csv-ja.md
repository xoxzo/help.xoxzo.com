...
項目が入力できたら、入力した列を選んで、フォーマットし、「テキスト」とします。
.csv 形式で保存します。

送信内容データを、入力したら、ファイルは完成です。
![image csv-04](/images/csv-04.png)

3. EZSMSのログインページより、「CSVでカスタマイズSMS送信」を選び、完成したファイルをアップロードし、送信を行ってください。
![image csv-05](/images/csv-05.png)

上記で使用した、サンプルCSVファイルは、[こちらよりダウンロード](ezsms_csvsample.csv)いただけます。
